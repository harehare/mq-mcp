...
EOF
    echo -e "${BOLD}${CYAN}  Model Context Protocol Server for mq${NC}"
    echo -e "${BLUE}  MCP server for processing Markdown and HTML with mq${NC}"
    echo ""
    echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
}
...
install_mq() {
    ...
    if [[ ! -d "$MQ_BIN_DIR" ]]; then
        mkdir -p "$MQ_BIN_DIR"
        log "Created installation directory: $MQ_BIN_DIR"
    else
        log "Installation directory already exists: $MQ_BIN_DIR"
    fi
    ...
    if [[ -n "$checksums_file" && -f "$checksums_file" ]]; then
        if ! verify_checksum "$temp_file" "$checksums_file" "$release_binary_name"; then
            rm -f "$checksums_file"
            warn "Checksum verification failed, but continuing installation"
        fi
        rm -f "$checksums_file"
    fi
...
update_shell_profile() {
    ...
    if [[ -n "$shell_profile" ]]; then
        ...
        if ! grep -q "$MQ_BIN_DIR" "$shell_profile" 2>/dev/null; then
            echo "" >> "$shell_profile"
            echo "# Added by mq-mcp installer" >> "$shell_profile"
...
show_post_install() {
    echo ""
    echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${BOLD}${GREEN}✨ mq-mcp installed successfully! ✨${NC}"
    echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    ...
    echo -e "     }${NC}"
    echo ""
    echo -e "${BOLD}${CYAN}⚡ Available MCP Tools:${NC}"
    echo -e "  ${GREEN}▶${NC} ${CYAN}html_to_markdown     # Convert HTML to Markdown${NC}"
    echo -e "  ${GREEN}▶${NC} ${CYAN}extract_markdown     # Execute mq queries on Markdown${NC}"
    echo -e "  ${GREEN}▶${NC} ${CYAN}available_functions  # List available functions${NC}"
    echo -e "  ${GREEN}▶${NC} ${CYAN}available_selectors  # List available selectors${NC}"
    echo ""
...
while [[ $# -gt 0 ]]; do
    case $1 in
        --help|-h)
            echo "mq-mcp installation script"
            echo ""
